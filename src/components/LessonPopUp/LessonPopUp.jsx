...
export const LessonPopUp = ({ lesson, hideFunc, mousePos }) => {
    ...
    return (
        <div className={`lesson-popup-container ${containerBackIsSolid ? 'lesson-popup-container-filled' : ''}`}>
            <div className="popup-hide-area" onClick={() => { hideFunc() }}></div>

            <div ref={popupWindowRef} style={{
                '--starting-x': `${xShift}px`,
                '--starting-y': `${yShift}px`,
                '--center-x': `${xCenter}px`,
                '--center-y': `${yCenter}px`,
            }} className={`popup-window border ${windowCentered ? 'popup-window-show' : ''}`}>
                <div className="head">
                ...
                <div className="data">
                    ...
                    <Record
                        label={"Замены"}
                        value={lesson.replaces}
                    />
                    <Record
                        label={"Модуль"}
                        value={lesson.getModule()}
                    />
                </div>
            </div>
        </div>
