...
export const Dropdown = ({ placeHolder, options, handleChoice, active }) => {
    ...
    return (
        // BUG: tabIndex={0} onFocus={toggleShowDropdown} onBlur={toggleShowDropdown} for keyboard works incorrectly
        <div ref={dropdownRef} className={`dropdown-container ${active ? 'dropdown-active' : ''}`}>
            <div className="dropdown-input" onClick={toggleShowDropdown}>
            ...
            {showDropdown && <div className='dropdown-menu'>
                {options.map((option) => (
                    <div key={option.value} className='dropdown-item' onClick={() => {
